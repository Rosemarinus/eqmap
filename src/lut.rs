...
define_language! {
    ...
    pub enum LutLang {
        ...
        "CYCLE" = Cycle([Id; 1]),
        // === 新增：Canonical 虚拟节点 ===
        // 它的结构和 LUT 一模一样：[ProgramID, Input0, Input1, ...]
        // 但我们用不同的名字，以便提取器 (Extractor) 能区分它们
        "CANON" = Canonical(Box<[Id]>),
    }
}
...
impl LutLang {
    ...
    fn verify(&self) -> Result<(), String> {
        match self {
            LutLang::Lut(list) | Self::Canonical(list) => {
                let l = list.len();
                if l == 0 {
    ...
    pub fn get_program(&self, expr: &RecExpr<Self>) -> Result<u64, String> {
        match self {
            LutLang::Lut(l) | LutLang::Canonical(l) => {
                self.verify()?;
    ...
    fn eval_rec(
        ...
        match self {
            ...
            LutLang::Lut(a) | LutLang::Canonical(a) => {
    ...
    pub fn get_prim_name(&self) -> Option<String> {
        match self {
            ...
            LutLang::Canonical(l) => Some(format!("CANON{}", l.len() - 1)),
            _ => None,
        }
    }
}
